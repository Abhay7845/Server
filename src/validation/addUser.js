...
const addUserValidation = [
  body("name", "Name is required").isLength({ min: 3 }).withMessage("not a valid name"),
  body("occupation", "Occupation is required").isLength({ min: 3 }),
  body("email", "Enter valid Email").isEmail().withMessage("not a valid email"),
  body("phone", "Enter valid Phone number").isLength({ min: 10 }).withMessage("not a valid phone number"),
